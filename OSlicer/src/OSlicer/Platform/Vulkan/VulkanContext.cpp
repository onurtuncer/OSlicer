...
namespace OSlicer {
...
#if defined(OS_DEBUG) || defined(OS_RELEASE)
	...
	static bool CheckDriverAPIVersionSupport(uint32_t minimumSupportedVersion)
	{
		...
		{
			OS_CORE_FATAL("Incompatible Vulkan driver version!");
			OS_CORE_FATAL("  You have {}.{}.{}", VK_API_VERSION_MAJOR(instanceVersion), VK_API_VERSION_MINOR(instanceVersion), VK_API_VERSION_PATCH(instanceVersion));
			OS_CORE_FATAL("  You need at least {}.{}.{}", VK_API_VERSION_MAJOR(minimumSupportedVersion), VK_API_VERSION_MINOR(minimumSupportedVersion), VK_API_VERSION_PATCH(minimumSupportedVersion));
	...
	{
		OS_CORE_INFO_TAG("Renderer", "VulkanContext::Create");

		OS_CORE_ASSERT(glfwVulkanSupported(), "GLFW must support Vulkan!");
...
#else
			OS_CORE_ERROR("Incompatible Vulkan driver version.\nUpdate your GPU drivers!");
#endif
			OS_CORE_VERIFY(false);
		}
