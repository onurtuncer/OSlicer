...
//#include "OSlicer/Editor/EditorConsole/EditorConsoleSink.h"

#include <filesystem>

#define OS_HAS_CONSOLE !OS_DIST
...
namespace OSlicer {
	...
	void Log::Init()
	{
		...
		oslicerSinks[0]->set_pattern("[%T] [%l] %n: %v");
		appSinks[0]->set_pattern("[%T] [%l] %n: %v");

#if OS_HAS_CONSOLE
		oslicerSinks[1]->set_pattern("%^[%T] %n: %v%$");
		appSinks[1]->set_pattern("%^[%T] %n: %v%$");
		for (auto sink : editorConsoleSinks)
			sink->set_pattern("%^%v%$");
#endif

		s_CoreLogger = std::make_shared<spdlog::logger>("CORE", hazelSinks.begin(), hazelSinks.end());
