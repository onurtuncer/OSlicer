...
namespace OSlicer {
	...
	static OSlicer::AllocationStats s_GlobalStats;
...
#ifndef OS_DIST
			if (!found)
				OS_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
#if OS_TRACK_MEMORY && OS_PLATFORM_WINDOWS

_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size)
{
	return OSlicer::Allocator::Allocate(size);
}
...
{
	return OSlicer::Allocator::Allocate(size, desc);
}
...
{
	return OSlicer::Allocator::Allocate(size, file, line);
}
...
{
	return OSlicer::Allocator::Free(memory);
}
