...
#include "OSlicer/Utilities/FileSystem.h"

#include "OSlicer/EntryPoint.h"

#ifdef OS_PLATFORM_WINDOWS
#include <Shlobj.h>
#endif

class Slicer35Application : public OSlicer::Application
{
public:
	HazelnutApplication(const OSlicer::ApplicationSpecification& specification, std::string_view projectPath)
		: Application(specification), m_ProjectPath(projectPath), m_UserPreferences(OSlicer::Ref<OSlicer::UserPreferences>::Create())
	{
	...
	virtual void OnInit() override
	{
		...
		PushLayer(new OSlicer::EditorLayer(m_UserPreferences));
	}
...
Hazel::Application* Hazel::CreateApplication(int argc, char** argv)
{
	...
	return new Slicer35Application(specification, projectPath);
}